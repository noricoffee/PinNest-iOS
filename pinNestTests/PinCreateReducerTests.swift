...
struct PinCreateReducerTests {
    ...
    func saveButtonTapped_createText_callsPinClient() async {
        ...
        await store.send(.saveButtonTapped(imageData: nil, videoPath: nil, pdfData: nil)) { state in
            state.isSaving = true
            state.saveError = nil
        }
    ...
    func saveButtonTapped_ignored_whileSaving() async {
        ...
        await store.send(.saveButtonTapped(imageData: nil, videoPath: nil, pdfData: nil))
    }
}
