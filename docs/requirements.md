...
> ⚠️ v1.0 のスコープ外。タグ機能で代替可能と判断し、将来バージョンで再検討する。

| # | 機能 | 優先度 | 備考 |
|---|------|--------|------|
| 3-1 | コレクションの作成 | ✕ | スコープ外（v1.0） |
| 3-2 | コレクションの編集 | ✕ | スコープ外（v1.0） |
| 3-3 | コレクションの削除 | ✕ | スコープ外（v1.0） |
| 3-4 | コレクション一覧の表示 | ✕ | スコープ外（v1.0） |
| 3-5 | コレクションへのピン追加・移動 | ✕ | スコープ外（v1.0） |
| 3-6 | コレクションの共有 | ✕ | スコープ外（v1.0） |
...
|--------|------|---------|
| スプラッシュ画面 | 起動時のローディング | - |
| ホーム画面 | 全種別混在のピン一覧（グリッド / リスト）・種別フィルタ | 2-4, 2-8 |
